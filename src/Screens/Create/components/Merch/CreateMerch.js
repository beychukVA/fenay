...
import { merchComponentsFactory } from "./MerchFactory";
import { ReviewAccountBanner } from "../ReviewAccountBanner";
import ArrowLeftWhite from "../../../../assets/ArrowLeftWhite.svg";

export const CreateMerch = ({ setShowMenu }) => {
  const [step, setStep] = useState(MerchStepsEnum.MERCH_DETAILS);
  ...
  const values = Object.values(MerchStepsEnum);
  const dots = [...values, ...values];
  let stepIndex = useMemo(() => {
    let index = values?.findIndex((value) => value === step);
    if (data?.royalities?.length > 0 && MerchStepsEnum.MERCH_ROYALITIES)
      index++;
    return index;
  ...
  const checkBackStep = () => {
    switch (step) {
      case MerchStepsEnum.MERCH_DETAILS:
        setShowMenu(true);
        break;
      case MerchStepsEnum.MERCH_ROYALITIES:
        setStep(MerchStepsEnum.MERCH_DETAILS);
        break;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/** Step Content */}
        {merchComponentsFactory.getComponent({
          step,
          data,
          setData,
          setStep,
