...
import { songComponentsFactory } from "./SongFactory";
import { ReviewAccountBanner } from "../ReviewAccountBanner";
import ArrowLeftWhite from "../../../../assets/ArrowLeftWhite.svg";

export const CreateSong = ({ setShowMenu }) => {
  const [step, setStep] = useState(SongStepsEnum.SONG_AUDIENCE);
  ...
  const values = Object.values(SongStepsEnum);
  const dots = [...values, ...values];
  let stepIndex = useMemo(() => {
    let index = values?.findIndex((value) => value === step);
    if (data?.royalities?.length > 0 && SongStepsEnum.SONG_ROYALITIES) index++;
    return index;
  ...
  const checkBackStep = () => {
    switch (step) {
      case SongStepsEnum.SONG_AUDIENCE:
        setShowMenu(true);
        break;
      case SongStepsEnum.SONG_DETAILS:
        setStep(SongStepsEnum.SONG_AUDIENCE);
        break;
      case SongStepsEnum.SONG_ROYALITIES:
        setStep(SongStepsEnum.SONG_DETAILS);
        break;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/** Step Content */}
        {songComponentsFactory.getComponent({
          step,
          data,
          setData,
          setStep,
